...
test("generate lightning invoice", () => {
	...
	new_invoice.paymentHash = Buffer.from("0001020304050607080900010203040506070809000102030405060708090102", "hex");
	...
	expect("").toBe(encoded_invoice);
...
test("parse lightning invoice", () => {
	...
	expect(decoded_invoice.signature).toBe("d7904cc4b74a22269c68c1df68a96c214d651b9376e9f164d3604da4b7deccce0e82aaab4c85d358ea14d0ae342da30812f95d976082eaac813911dae01af3c1");
	if(decoded_invoice.tags) {
		let payment_hash = decoded_invoice.tags.filter((tag) => tag.tagName === "payment_hash"); // should only be one payment_hash tag
		if(payment_hash) expect(payment_hash[0].data).toBe("0001020304050607080900010203040506070809000102030405060708090102")

		let description = decoded_invoice.tags.filter((tag) => tag.tagName === "description"); // should only be one payment_hash tag
